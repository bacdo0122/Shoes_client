                ...
                <Slider {...settings}>

                    {
                        product_category && product_category.map(value => (
                            <div key={value?._id} className="col-lg-12 animate__animated animate__zoomIn col_product" style={{ zIndex: '999', height: '30rem' }}>
                                <div className="single-product-wrap">
                                    <div className="product-image">
                                        <Link to={`/detail/${value?.id_product?._id}`}>
                                            <img src={value && `http://localhost:8000/${value?.id_product?.image}`} alt="Li's Product Image" style={{maxHeight: '400px', objectFit: "contain"}}/>
                                        </Link>
                                        <span className="sticker">-{value && value.promotion}%</span>
                                    </div>
                                    <div className="product_desc">
                                        <div className="product_desc_info">
                                            <div className="product-review">
                                                <h5 className="manufacturer">
                                                    <a href="shop-left-sidebar.html">{value && value?.id_product?.name_product}</a>
                                                </h5>
